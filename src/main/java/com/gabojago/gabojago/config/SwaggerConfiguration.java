package com.gabojago.gabojago.config;
...
//	http://localhost[:8080]/{your-app-root}/swagger-ui/index.html
	...
	private String title = "가보자GO " + version;
...
//				.termsOfServiceUrl("https://edu.ssafy.com")
	...
	public Docket adminBoardApi() {
		return getDocket("공지사항", Predicates.or(PathSelectors.regex("/admin.*")));
	}

	@Bean
	public Docket userApi() {
		return getDocket("사용자", Predicates.or(PathSelectors.regex("/user.*")));
	}

	@Bean
	public Docket tripsearchApi() {
		return getDocket("여행지조회", Predicates.or(PathSelectors.regex("/tripsearch.*")));

	}
	@Bean
	public Docket userboardApi() {
		return getDocket("일반게시글", Predicates.or(PathSelectors.regex("/userboard.*")));
